...
use crate::scanner::passive::{PassiveScanner, ArpMonitor, ArpEvent};
use crate::scanner::passive::mdns::PassiveDevice;
use crate::monitor::events::{NetworkEvent, DeviceSnapshot};
use tokio::sync::mpsc;
use std::sync::Arc;
...
) -> Result<(mpsc::Receiver<PassiveDevice>, Option<mpsc::Receiver<ArpEvent>>), Box<dyn std::error::Error>> {
    // mDNS listener
...
pub fn passive_device_to_snapshot(device: PassiveDevice) -> DeviceSnapshot {
    ...
    let device_type = device.device_type_hint
        .unwrap_or_else(|| "Unknown".to_string());
    
    DeviceSnapshot {
        mac: device.mac.unwrap_or_else(|| format!("unknown_{}", device.ip)),
        ip: device.ip,
...
pub fn handle_mdns_device<F>(
    device: PassiveDevice,
    callback: &Arc<F>,
) where
    F: Fn(NetworkEvent) + Send + Sync,
{
    let device_type = device.device_type_hint.clone()
        .unwrap_or_else(|| "Unknown".to_string());
